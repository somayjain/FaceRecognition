    ...
    function verification(test_image, claim_label)
        ...
        p = 32; q = 32;
        k = 10;
        verify(CMUPIEData, 0, p, q, k, test_image, claim_label)
    end
     
    function [] = verify(CMUPIEData, fold_no, p, q, k, test_image, claim_label)
        ...
        for i=1:NumImgs
            ...
            if(mod(i,4) == fold_no)
                ...
                TFV(cnt,:) = testFV;
                test_label(cnt) = CMUPIEData(i).label;
                cnt = cnt + 1;
            end
        ...
        while ( b_cnt < size(TFV,1) )
            ...
            ROC_labels(cnt) = 1;
            dist = 0.0;
            assert(labels(a_cnt)==test_label(b_cnt));
            for z = 1:k
        ...
        if(minn > req_thresh)
            ...
            if(str2num(claim_label) == labels(mindx))
                disp('Yes');
            else
                disp('No');
            end
